 ...
 * Run form project folder. Once you have build with cmake, use mpirun -np [p] ./build/mpi [file name] [timesteps] [iches of rain] 
 * 
...
)
{
    ...
    int total_water_rows = grid_height + top_ghost_row_count + bottom_ghost_row_count;
    ...
    for (int y = 0; y < elev_height; y++)
    {
        for (int x = 0; x < elev_width; x++)
        {
            ...
            //ghost aware y
            int ghost_y = y + top_ghost_row_count;
            ...
            if (x > 0 && (ghost_y - 1) >= 0)
            {
                total_water += water_levels[(ghost_y - 1) * grid_width + (x - 1)];
                cells_counted++;
            }
            // Northeast cell
            if (x < grid_width && (ghost_y - 1) >= 0)
            {
                total_water += water_levels[(ghost_y - 1) * grid_width + x];
                cells_counted++;
            }
            // Southwest cell
            if (x > 0 && ghost_y < total_water_rows)
            {
                total_water += water_levels[ghost_y * grid_width + (x - 1)];
                cells_counted++;
            }
            // Southeast cell
            if (x < grid_width && ghost_y < total_water_rows)
            {
                total_water += water_levels[ghost_y * grid_width + x];
                cells_counted++;
            }
    ...
    int p)
{
    ...
    for (int i = 0; i < num_timesteps; i++)
    {
        ...
        compute_water_surface_elevations(elevation, in, surface_elevations, elev_width, elev_height, top_ghost_row_count, bottom_ghost_row_count);
...
void print_row_sample(const char *label, double *arr, int row_index, int width, int rank)
{
    std::cout << "Rank " << rank << " " << label << " row " << row_index << ": ";

    int limit = std::min(8, width);
    for (int x = 0; x < limit; x++)
    {
        std::cout << arr[row_index * width + x] << " ";
    }
